...
class TubeViT(nn.Module):
    def __init__(
        ...
        if representation_size is None:
            heads_layers["head"] = nn.Linear(self.hidden_dim, 101)
        else:
            heads_layers["pre_logits"] = nn.Linear(self.hidden_dim, representation_size)
            heads_layers["act"] = nn.Tanh()
            heads_layers["head"] = nn.Linear(representation_size, 101)

        self.heads = nn.Sequential(heads_layers)
        #self.fc1 = nn.Linear(101, 30)
        self.fc1 = nn.Linear(101, self.num_classes)
