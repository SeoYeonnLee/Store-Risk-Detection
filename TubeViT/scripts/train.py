...
def main(
    ...
    lambda x: x.permute(1, 0, 2, 3), # T, C, H, W
    Resize(size=video_size),
    Normalize(mean=imagenet_mean, std=imagenet_std),
    ...
    lambda x: x.permute(1, 0, 2, 3) # C, T, H, W
    ])
    ...
    train_dataframe = './temp_train_new.csv'
    ...
    val_dataframe = './temp_val_new.csv'

    val_set = MyUCF101(
        mode='validation',
        root="/mnt/d/data/temp_val_new",
        annotation_path="/mnt/d/data/temp_val_new",
